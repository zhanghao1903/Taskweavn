...
| Diagnostics descriptors | In progress in `codex/product-1-1-diagnostics-inspection-summary`: workspace inspection evidence diagnostic output now includes a support-oriented summary with evidence counts, latest capture time, preview limits, safe-use guidance, and explicit redaction limitations. Per-route Electron log descriptors remain a separate follow-up. | Improves supportability beyond the P0 route-matrix closure. |
