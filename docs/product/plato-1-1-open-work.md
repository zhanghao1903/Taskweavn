...
| Web retrieval beta hardening | In progress: an opt-in live Tavily Search/Extract smoke runbook and pytest harness now exist for release operators with a real key. Remaining work: run and attach live-key evidence, broader citation/result UI, Audit projection depth, user-visible limitations, and future retrieval budget boundaries. | Search/fetch are implemented; the remaining work is beta trust and release evidence, not first capability delivery. |
