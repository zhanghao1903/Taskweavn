...
| Web retrieval beta hardening | Verify real Tavily Search/Extract smoke, broader citation/result UI, Audit projection depth, user-visible limitations, and implement the scoped [Web Retrieval Budget Boundary](../plans/feature/web-retrieval-budget-boundary.md). | Search/fetch are implemented; the remaining work is beta trust, cost transparency, and release evidence, not first capability delivery. |
