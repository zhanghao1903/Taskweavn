...
| Web retrieval beta hardening | User-visible beta limitations are now defined in [Plato 1.1 Web Retrieval Beta Limitations](plato-1-1-web-retrieval-beta-limitations.md). Remaining hardening: real Tavily Search/Extract smoke, broader citation/result UI, Audit projection depth, and future retrieval budget boundaries. | Search/fetch are implemented; the remaining work is beta trust and release evidence, not first capability delivery. |
