# Plato Public Exposure

> Status: active planning area
> Last Updated: 2026-06-24
> Scope: product, architecture, release, and visual planning for public-facing
> Plato material in `zhanghao1903/plato-public`.

This directory keeps public-exposure planning together under the Plato product
docs tree. It is for deciding what can be shown publicly, what needs rewriting
or sanitization, and which visual assets must be produced before public docs are
expanded.

| File | Purpose |
|---|---|
| [plato-public-exposure-plan.md](plato-public-exposure-plan.md) | Public repository exposure strategy, including product narrative, architecture boundaries, information architecture, execution slices, risks, and assumptions. |
| [plato-1-1-public-release-doc-sync.md](plato-1-1-public-release-doc-sync.md) | Product 1.1 beta public release docs sync package: source docs, public targets, allowed/forbidden claims, limitations, and public PR checklist. |
| [plato-public-repository-optimization-plan.md](plato-public-repository-optimization-plan.md) | Implementation-ready optimization plan for making `zhanghao1903/plato-public` more user-friendly, recruiter-friendly, and release-ready. |
| [public-visual-asset-gaps.md](public-visual-asset-gaps.md) | Image, screenshot, diagram, and visual presentation gap tracker for public Plato materials. |

Product 1.1 beta source release notes are now prepared in
[Product 1.1 Beta External Release Notes](../../releases/product-1-1-beta-external-release-notes.md).
Treat that file as the public-safe source for release-copy and known-limitations
sync; the actual public repository update remains a separate publishing
operation.

Do not copy private implementation docs, screenshots, logs, workspaces, or
diagnostics into the public repository until the relevant item is approved by
the exposure plan and visual gap checklist.
