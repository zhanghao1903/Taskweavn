...
| Electron workspace-entry smoke | Release-readiness smoke for the Product 1.0 Workspace Picker W1 path. | `cd frontend && npm run electron:smoke:workspace-entry`. | Seeded recent workspace data validates Workspace Picker -> selected workspace -> main-owned sidecar startup -> Main Page/Audit/Diagnostics/recovery paths, without exposing the raw workspace path in the renderer. |
...
| Workspace Picker / root semantics | Workspace Picker W1 is accepted as the Product 1.0 desktop entry when no workspace is persisted. W2 workspace-root-as-agent-cwd semantics are implemented: new session metadata lives under `.taskweavn/sessions/<session_id>`, Agent file writes land at workspace root, and normal workspace tools reject `.taskweavn` access. `npm run electron:smoke:workspace-entry` covers recent workspace selection in the Electron dev shell, sidecar startup from the selected workspace, transition into Main Page, and path redaction. |
