...
### 7.10 SessionActivityTimelineResult

`SessionActivityTimelineResult` 是 Session Conversation / Activity Timeline 的只读 typed projection。它把已有安全 UI facts 投影成用户可追溯的 timeline item，给后续 Runtime Input Router 的问答、节点属性修订、执行请求等 decision 提供统一显示和审计入口。
...
- Activity 只消费 `SessionMessageView`、`PlanView`、Task projection、ASK/confirmation projection、result/file summary 等安全 UI facts；
- Activity 不暴露 provider payload、prompt、raw EventStream row、tool arguments、SQLite row、secret 或 raw absolute workspace path；
- `scopeKind=task` 必须带 `taskNodeId`，`scopeKind=plan` 必须带 `planId`；
- Router 产生的 `router_interpretation`、`guidance_recorded`、`execution_request` 等 item 预留到 Runtime Input Router slice。

### 7.11 ConfirmationActionView
...
| `GET` | `/api/v1/sessions/{sessionId}/activity?limit=50&cursor=...` | `SessionActivityTimelineResult` | Session Conversation / Activity Timeline 的 typed projection。 |
