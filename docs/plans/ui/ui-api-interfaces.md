...
| `ObjectRef` | 非 Task 对象引用：RawTask、RawTaskAsk、draft tree/subtree、message、command |
...
- `ObjectRef` 不替代 `TaskRef`，只覆盖 TaskRef 表达不了的 authoring/system objects；
- `AffectedObjectRef` 可表达 `created`、`replaced`、`superseded`、`needs_review` 等 command impact；
- UI 如需对象内容，应通过 snapshot/detail/query 重查，不从 `ObjectRef` 推断业务内容。
...
`id` 是 projection/cache/debug id。第一版允许 synthetic id；普通产品 UI 不展示该 id，也不把它当成 domain primary key。
...
`affected_task_refs` 只承载 draft/published Task。RawTask、RawTaskAsk、draft tree、draft subtree 等非 Task 对象通过 `ObjectRef` / `AffectedObjectRef` 表达。UI 不应仅依赖本地 optimistic state。命令 accepted 后，应等待订阅事件或重新查询 ViewModel。

`updateTaskNode` 应携带 `update_mode`：

- `node_fields`：只修改当前节点字段；
- `replace_children`：父节点保留，直接 children 可重建；
- `replace_subtree`：父节点 id 默认保留，descendants 可整体重建。
...
| 接口 | 返回 | 最低能力 |
|---|---|---|
| `appendSessionInput(mode="generate_task_tree")` | `CommandResult` + `TaskTreeView` refresh | Main Page 主工作流：从自然语言进入 RawTask authoring flow |
| `generateTaskTree(raw_task_id, context)` | `CommandResult` + `TaskTreeView` refresh | 高级/特定工作流入口：从 ready RawTask 显式生成 Draft Task Tree |
