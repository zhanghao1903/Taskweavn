...
> Status: aligned with Phase 3C Slice 6
...
| `appendTaskMessage` | `TaskCommandService.append_task_message` |
| `resolveConfirmation` | `TaskCommandService.resolve_confirmation` |
| `publishTaskTree` | `TaskCommandService.publish_task_tree` + `TaskPublisher` |
| `retryTask` | `TaskCommandService.retry_task` + `TaskPublisher` |

后续需要补充：

- `generateTaskTree`：由 Collaborator Agent + DraftTaskStore 实现；
- `appendSessionMessage`：Session-level MessageBus command；
