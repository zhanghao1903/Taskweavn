...
> Related Docs: `docs/architecture/authoring-domain.md`, `docs/architecture/agent.md`, `docs/architecture/task.md`, `docs/architecture/bus.md`, `docs/architecture/task-domain-ui-model-separation.md`, `docs/plans/task-first-ui-interaction.md`
...
User global input
  ...
  → CreateRawTaskTool creates RawTask
  → AssessRawTaskFeasibilityTool evaluates feasibility / missing inputs
  → if needs clarification: AskRawTaskQuestionTool
...
RawTask ready_to_plan
  ...
  → GenerateDraftTaskTreeTool creates draft root tasks from RawTask
...
### 7.2 AssessRawTaskFeasibilityTool
...
### 7.6 ProposeTaskNodeOptionsTool
...
### 7.10 AppendTaskAuthoringMessageTool
...
AgentTemplate(
    ...
    tools=[
        CreateRawTaskTool,
        AssessRawTaskFeasibilityTool,
        AskRawTaskQuestionTool,
...
appendSessionMessage(global user input)
  ...
  → AssessRawTaskFeasibilityTool
  → if needs clarification: AskRawTaskQuestionTool
  → if ready_to_plan: GenerateDraftTaskTreeTool
...
|---|---|
| `generateTaskTree` | `CreateRawTaskTool` + `AssessRawTaskFeasibilityTool` + maybe `GenerateDraftTaskTreeTool` |
...
### Slice 3: In-Memory Authoring Store
...
### Slice 5: Collaborator Authoring Service
...
- 所有 durable 输出写入 DraftTaskStore / MessageStream。

### Slice 6: PublishDraftTasksTool and TaskPublisher Boundary
...
### Slice 7: Collaborator Agent Template And API Adapter
...
- capability=`task_authoring`
- Session start 注册逻辑
- `generateTaskTree` / `appendSessionMessage` / `appendTaskMessage` / `publishTaskTree` adapter
...
- Completed in first implementation pass:
  ...
  - Added tests for authoring context, proposal schemas, option schemas, validation results, capability catalog, validator errors/warnings, and frozen model behavior.
- Verified:
  - `uv run pytest tests/test_task_authoring.py` — 13 passed, 1 warning
