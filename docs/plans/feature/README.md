...
| [message-ask-confirmation-backend.md](message-ask-confirmation-backend.md) | In progress: C1-C5 done for confirmation hardening, durable ASK store, TaskBus `waiting_for_user`, ASK commands/queries, snapshot projection, and answer-before-resume dispatch; runtime ASK tool and Context Manager ASK facts remain. |
| [message-ask-confirmation-backend-technical-design.zh-CN.md](message-ask-confirmation-backend-technical-design.zh-CN.md) | In progress 中文详细技术方案：durable ASK store、TaskBus `waiting_for_user`、answer-before-resume、confirmation 幂等和语义修正；C6 runtime/context 剩余。 |
