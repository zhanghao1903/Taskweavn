...
| Message, ASK, and confirmation UI integration | Interaction / UI | P0 | unplanned | [interaction-layer](../architecture/interaction-layer.md), [ui-backend-communication](../architecture/ui-backend-communication.md), [task-domain-ui-model-separation](../architecture/task-domain-ui-model-separation.md), [ADR-0014](../decisions/ADR-0014-interaction-control-taxonomy-for-product-1-0.md) | [UI API interfaces](../plans/ui/ui-api-interfaces.md), [ASK lifecycle contract](../engineering/ask-lifecycle-contract.md), [ASK interaction model](../interaction-model/ask-user-interaction.md) | Product 1.0 closure gap. Message substrate exists, but ASK is not yet implemented as a durable `ask_user` tool/action, `waiting_for_user` task pause, Main Page ASK Dock, answer command, event/refetch path, or restart-resumable user-input loop. Confirmation remains a separate authorization lifecycle. ADR-0014 keeps ASK/confirmation separate from interruption and defers this gap until after cooperative interruption; split it into smaller follow-up plans before implementation. |
