...
Task 路由不应该硬编码在 TaskBus 内。路由策略会涉及 capability、工具权限、成本、用户偏好、历史成功率、当前负载、特殊 Agent 能力，以及未来高级用户自定义策略。这个空间很难一次性做完整，所以系统把路由策略交给 **Router**，其中可以包含可插拔的 **Routing Agent policy**：
...
- Router 已写入 `assigned_agent_id`
- Agent Manager 为对应 Execution Agent 申请领取该 Task
...
- **与 Router：** Router 为 pending Task 提交 assignment command，不直接改状态；Routing Agent 是可插拔 policy
- **与 Agent Manager / Execution Agent：** Task 描述需求，Agent Manager 创建实例并领取被分配的 Task，Execution Agent 执行
