...
`intent` 给 Agent（人类可读），`required_capability` 给 Routing Agent 和执行 Agent（机器可读），`assigned_agent_id` 记录路由结果，`result` 给后续任务（结构化或自由文本）。
...
| `assigned_agent_id` | `AgentId \| None` | 当前被 Routing Agent 分配的执行 Agent。未分配时为 None |
| `assigned_by` | `AgentId \| SystemId \| None` | 提交 assignment command 的 Routing Agent 或系统策略 |
...
Task 路由不应该硬编码在 TaskBus 内。路由策略会涉及 capability、工具权限、成本、用户偏好、历史成功率、当前负载、特殊 Agent 能力，以及未来高级用户自定义策略。这个空间很难一次性做完整，所以系统把路由策略交给可插拔的 **Routing Agent**：

```text
pending Task
  -> Routing Agent decides responsibility
  -> AssignmentCommand
  -> TaskBus validates and records assignment
  -> assigned Execution Agent claims task
```

Routing Agent 可以很简单，例如完全硬路由；也可以使用 LLM 和补全/fallback 策略。高级用户未来可以接入自定义 Routing Agent。无论 Routing Agent 多灵活，它都不能直接修改 Task 状态，只能提交 assignment command。
...
- 路由失败不会引入新状态，Task 保持 `pending`，并通过事件/消息/诊断说明原因。
...
```
pending → running   已分配的 Agent 领取任务
                    parent 必须是 done（否则任务停留在 pending）
                    claimant 必须匹配 assigned_agent_id
...
- Routing Agent 已写入 `assigned_agent_id`
- 对应 Execution Agent 申请领取该 Task

匹配成功后，`status: pending → running`。

如果没有可用 Agent，Task 保持 `pending`。系统可以写入 routing notice 或用户消息，例如 "No available Agent can handle capability X"，但应做节流，避免频繁打扰用户。
...
- **与 Routing Agent：** Routing Agent 为 pending Task 提交 assignment command，不直接改状态
- **与 Execution Agent：** Task 描述需求，Execution Agent 领取被分配给自己的 Task 并执行
