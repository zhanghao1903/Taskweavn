...
| Task domain and TaskBus | [task.md](task.md), [bus.md](bus.md), [bus-v2.md](bus-v2.md) | Task lifecycle, TaskBus authority, `waiting_for_user` ASK blocking point, retry/skip, cooperative interruption, Routing Agent assignment direction. |
...
| UI/backend boundary | [ui-backend-communication.md](ui-backend-communication.md), [task-domain-ui-model-separation.md](task-domain-ui-model-separation.md) | Main Page snapshot, Session conversation, Query/Command/Event, ASK/confirmation, frontend/backend contract direction. |
