...
> 多 Agent 协作架构的核心抽象 · v1.5 · 2026-07-10
>
...
>
> 2026-07-10 fact calibration: Product 1.1 当前新增了本地 Execution Plane / Task API foundation。`EmbeddedTaskApiService` 可以接受 `TaskRequest`，普通 task type 仍进入 TaskBus 和 fixed-route dispatcher；特定 task type 可以通过 local `EmbeddedTaskRuntimeHandler`（例如 WeChat send runtime）执行受控本地流程。该 runtime handler seam 不是 public Agent Manager，也不是 dynamic Agent assignment。
...
当前 Product 1.1 本地闭环仍不把 Agent Manager、public Agent protocol 或自定义
Agent 注册暴露给用户。它保留 Product 1.0 固定 Default Agent 路由，同时新增
Runtime Input Router 和本地 Execution Plane / Task API facade。Agent template /
run 分层仍然保留，便于后续接入 Routing Agent、Agent Manager、skills 和 MCP。
注意：当前 Runtime Input Router 是用户输入/Contract Revision 边界，不是
TaskBus 的 Routing Agent assignment loop。
...
当前实现没有通用 `CreateTaskTool`。协作式任务创建和修订由内置 Collaborator
Authoring Service、Authoring Command Service、Contract Revision Commands 和
TaskPublisher 边界完成。后续 public Agent protocol 可以把“创建/修订任务”暴露为
command-backed tool-like capability，但该能力必须继续通过命令层验证和持久化，
不能让 Agent 直接改 TaskBus 或 Plan/TaskNode store。

### 2.5 Routing Agent 是可插拔策略对象

Task 路由策略不写死在 TaskBus。后续 dynamic routing 中，Routing Agent 负责决定 pending Task 应该交给哪个 Execution Agent：
...
Product 1.1 current note: Runtime Input Router 可以把用户输入分发到 read-only
inquiry、guidance、ASK/confirmation resolution 或 execution handoff，但它仍不是
本节的 Routing Agent assignment policy。当前 TaskBus claim 仍按
`required_capability` 和 fixed Default Agent identity 推进。
...
当前已实现的 Agent-adjacent metadata 只覆盖局部边界：

- `CollaboratorAgentTemplate` 是内置 Collaborator 的 session-scoped metadata，
  不挂载 workspace/shell tools，真实写入仍走 Authoring Commands；
- `StaticCapabilityCatalog` / `AgentCapabilityCatalog` 用于 authoring 和 publish
  validation，不是动态 AgentPool；
- `AgentLlmRole` / `SettingsBackedAgentLlmResolver` 用于 backend-only role-aware
  LLM client selection，不创建 runtime Agent instance。
...
本节是目标抽象。当前没有通用 public `AgentTemplate` registry；具体实现只有内置
Collaborator template metadata、CapabilityCatalog、Agent LLM role/profile 解析和
fixed Default Agent boundary。
...
本节同样是目标抽象。当前 fixed-route path 的代码事实是
`AgentLoopResidentDefaultAgent.run(task)` 为每个 Task 创建或取得一个
AgentLoop-compatible runner，运行结束后映射为 `TaskRunResult`；dispatcher 不在
Task 之间保留 AgentLoop instance。
...
两个 audit Agent 可以有不同的工具集：
  ...
  AuditTemplateB.tools = [read_file, diff, propose_fix_task] # 审计 + 派生修复任务

它们的 capability 都是 "audit"，调度上等价
但实际能力差异巨大
```

当前实现没有通用 `CreateTaskTool`。这里的派生任务能力如果进入 public Agent
protocol，必须落到 command-backed authoring / publish 边界，而不是工具直接写
TaskBus。
...
Product 1.1 当前本地闭环使用内置 Default Agent boundary，不要求 AgentPool。
内置 Collaborator 只有 metadata registry，用于说明系统 Collaborator 已就绪。
后续动态 Agent Template 可以在 Session 启动时（或更早）注册到 AgentPool：
...
当前本地 Product 1.1 路径中，`FixedRouteExecutionDispatcher` 接收 publish、
retry、ASK answer resume 或 Task API publish 后的 dispatch trigger，并为对应
Session 创建 `FixedRouteTaskExecutor` tick。当 `FixedRouteTaskExecutor` 从
TaskBus claim 到某个 Task，它启动一个 task-scoped Default Agent run。

本地 Execution Plane 的普通 task type 也会先映射到 TaskBus Task，再进入这条
fixed-route path。特定 task type 可以由 `EmbeddedTaskRuntimeHandler` 接管本地
流程；这属于 service/runtime handler seam，不是 Agent Manager 创建通用 Agent。

后续 Product 1.1+ / 1.2 中，Agent Manager 可从对应 capability 的 Template 创建
runtime / run：
...
当前 Default Agent run 由 `AgentLoop` 执行：它注册 workspace file/search/shell
tools、可选 `web_search` / `web_fetch`、可选 `computer_use`、`ask_user` 和
`request_confirmation`，并通过 Context Manager 在每次 LLM call 前构建
cache-aware context。

```python
def execute(self) -> TaskResult:
    # 1. 构造 prompt（system + task + workspace context + explicit guidance）
...
下图描述 Product 1.1+ routed Agent model。Product 1.0 / 当前 Product 1.1 fixed-route path 由
...
```
        ...
        AgentRun (task-scoped)
          ...
          │  └── calls → LLM
          │
          └── future may request → authoring / publish command
                                      │
                                      ↓
                                    TaskBus
```
...
- **与 Execution Plane：** 当前本地 Task API 是 service facade。普通 task type 映射到 TaskBus/fixed-route Agent run；特定 runtime handler 可以处理受控本地流程，但不等于 public Agent Manager
- **与 Workspace：** Agent 通过工具读写 selected workspace root，无独立工作区
- **与 Tool：** Tool 是 Agent 能力的载体；同一个 Tool 可被多个 Template 共用
- **与 ThoughtStore：** ThoughtStore 协议和 SQLite/Null 实现存在，但当前 sidecar Default Agent path 没有把它作为用户可见长期记忆或事实权威
...
| 经验复用 | 未来通过 ThoughtStore + 显式注入 | AgentRun 间共享私有状态 | 数据是数据，行为是行为 |
