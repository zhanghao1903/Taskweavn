...
* :meth:`SandboxExecutor.execute` writes the snippet (plus a tiny
  instrumentation wrapper) into ``<workspace>/.code-agent/runs/<event_id>/``,
...
workspace (skipping ``.code-agent/`` so our own bookkeeping doesn't show up
...
class SandboxConfig:
    ...
    container_name_prefix: str = "code-agent-sandbox"
