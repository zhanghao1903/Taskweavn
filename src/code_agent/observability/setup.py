...
Four named loggers route to four files:
    ...
    code_agent.tool        -> tool.log         (Runtime dispatch + result + duration)
    code_agent.action      -> action.log       (every Action that lands on EventStream)
    code_agent.observation -> observation.log  (every Observation that lands on EventStream)
    code_agent.llm         -> llm.log          (LLM request + response)
